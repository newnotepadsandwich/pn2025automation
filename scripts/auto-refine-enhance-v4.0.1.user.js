// ==UserScript==
// @name         Auto Refine, Enhance, Inscribe & Recast 4.0.1
...
/*
═══════════════════════════════════════════════════════════════════════════════════════════════
⚔️ POCKIE NINJA AUTO REFINE, ENHANCE, INSCRIBE & RECAST 4.0.1
═══════════════════════════════════════════════════════════════════════════════════════════════
...
(function() {
    ...
    function checkStats() {
        return new Promise(resolve => {
            ...
            // Use dynamic timing based on speed setting, but ensure minimum time for tooltip to appear
            const tooltipDelay = Math.max(300, Math.min(500, refineSpeed));
            setTimeout(() => {
    ...
    function reroll() {
        return new Promise(resolve => {
            ...
            if (attemptButton) {
                ...
                setTimeout(() => {
                    // Find Accept button efficiently for ultra-fast mode
                    let acceptButton;
                    
                    // Quick search for Accept button by text
                    ...
                    // Fallback: search in different containers if quick search fails
                    if (!acceptButton) {
                        for (let i = 4; i <= 10; i++) {
                            const container = document.querySelector(`#game-container > div:nth-child(${i})`);
                            if (container) {
                                const buttons = container.querySelectorAll("button");
                                for (let btn of buttons) {
                                    if (btn.innerText.trim().toLowerCase() === "accept") {
                                        acceptButton = btn;
                                        break;
                                    }
                                }
                ...
                }, Math.max(200, refineSpeed / 2)); // Use dynamic timing based on speed setting
            } else {
    ...
    while (isRunningRefine) {
        ...
        if (hasRequired) {
            if (optionalCount >= 1) {
                ...
                console.log("⚠️ All required stats present, but no optional stats! Rerolling again...");
            }
        } else {
...
function setTalismanByLevel(currentLevel) {
    ...
    if (currentLevel >= 0 && currentLevel <= 5) {
        ...
        highRadio.click();
        setInputValue(inputBox, 3);
        console.log("🎯 Set 3 High Talisman for Level 7");
        ...
        setTimeout(() => {
            ...
            if (resultElem) {
                ...
                if (failStreak >= maxFailStreak) {
                    stopInscribe();
                    alert("⚠️ Auto Inscribe stopped due to consecutive failures!");
                }
            }
        }, 1000);
    } else {
...
function autoRecast() {
    ...
    if (attemptButton) {
        ...
        setTimeout(() => {
            ...
            setTimeout(autoRecast, enhanceSpeed); // continue loop after delay
        }, enhanceSpeed);
    } else {
    ...
    function createUI() {
        ...
        function updateButtons() {
            ...
            if (dropdown.value === "refine") {
                ...
                if (isRunningRefine) {
                    createButton("Stop Reroll", stopReroll, true);
                } else {
                    createButton("Start Reroll", autoReroll);
                }
                createButton("Check Stat", checkStats);
            } else {
                statsConfig.style.display = "none";
                if (dropdown.value === "enhance") {
                    if (isRunningEnhance) {
